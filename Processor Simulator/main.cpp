...
int main(int argc, char *argv[]) {
  ...
  time = 0;
  // bool processorAvailable = true;
  Process *runningProcess = nullptr;         // Pointer
  auto currentProcess = processList.begin(); // Iterator
  bool processesRemaining = true;

  // keep running the loop until all processes have been added and have run to
  // completion
  while (processMgmt.moreProcessesComing() ||
         processesRemaining) /* TODO add something to keep going as long as
      there are processes that arent done! */
  {
    ...
    if (runningProcess) { //  Is a Process Currently Running?
      runningProcess
          ->processorTime++; // Update Processor Time for the Running Process
      if (!runningProcess->ioEvents.empty() &&
          runningProcess->ioEvents.front().time ==
              runningProcess->processorTime) { // Does Running Process have IO
                                               // Event at this time step?
        stepAction = ioRequest;                // Issue IO Request
        ioModule.submitIORequest(time, runningProcess->ioEvents.front(),
                                 *runningProcess);
        runningProcess->ioEvents.pop_front();
        runningProcess->state = blocked; // Block the Running Process
        runningProcess = nullptr;
        // processorAvailable = true;
      } else if (runningProcess->processorTime ==
                 runningProcess->reqProcessorTime) { // Has the Process Run Long
                                                     // Enough to Complete?
        stepAction = complete; // Mark Process as Finished Running
        ...
        stepAction = continueRun; // No Events, Continue Process Run
      }
    } else { // No Process is Currently Running
      if (processList.back().state ==
          newArrival) { // Are there any newArrival Processes?
        stepAction =
            admitNewProc; // Admit New Process by Setting State to Ready
        for (auto it = processList.begin(); it != processList.end();
             it++) { // Find Process And Move to Ready State
          if (it->state == newArrival) {
            it->state = ready;
            break;
          }
        }
      } else if (!interrupts.empty()) { // Are There Any Pending Interrupts?
        ...
        interrupts.pop_front();               // Remove Interrupt
      } else if (runningProcess == nullptr) { // Select Process to Run (FIFO)
        auto start = currentProcess;
        ...
        while (true) {
          if (currentProcess->state == ready) {
            ...
            if (currentProcess == processList.end()) {
              currentProcess = processList.begin(); // Wrap around
            }
            break;
          }
