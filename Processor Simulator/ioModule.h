...
struct IOInterrupt {
  ...
  IOInterrupt(const unsigned int &eId, const unsigned int &pId)
      : ioEventID(eId), procID(pId) {};
...
public:
  ...
  inline void submitIORequest(const int &curTimeStep, const IOEvent &ioEvent,
                              const Process &proc) {
    m_pending.push_back(make_pair(curTimeStep + ioEvent.duration,
                                  IOInterrupt(ioEvent.id, proc.id)));
  }
